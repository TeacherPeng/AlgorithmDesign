...
// 计算点表中指定两点间距离的平方
inline double Distance2(const vector<pair<double, double>> &points, int i, int j)
{
    double x0 = points[i].first, y0 = points[i].second;
    double x1 = points[j].first, y1 = points[j].second;
    return Distance2(x0, y0, x1, y1);
}
...
double ShortestDistance(const vector<pair<double, double>> &points, const vector<pair<int, int>> xIndex, const vector<int> &yIndex, int s, int t)
{
    ...
    if (s + 1 == t)
        return Distance2(points, xIndex[s].first, xIndex[t].first); // 只有两个点时
    ...
    for (int i = k; i >= 0; i--)
    {
        ...
        for (int j = xIndex[i].second; j >= 0; j--)
        {
            // 略过左半区间的点
            if (yIndex[j] <= k)
                continue;
            ...
            if (y0 - y1 >= d)
                break;
            double d0 = Distance2(x0, y0, x1, y1);
            if (d0 < d)
                d = d0;
        }
...
double ShortestDistance(const vector<pair<double, double>> &points)
{
    ...
    for (int i = 0; i < points.size(); i++)
        xIndex[i].first = i;
    sort(xIndex.begin(), xIndex.end(), [points](pair<int, int> a, pair<int, int> b) { return points[a.first].first < points[b.first].first; });

    vector<int> yIndex(points.size());
    for (int i = 0; i < points.size(); i++)
        yIndex[i] = i;
    sort(yIndex.begin(), yIndex.end(), [points, xIndex](int a, int b) { return points[xIndex[a].first].second < points[xIndex[b].first].second; });

    for (int i = 0; i < yIndex.size(); i++)
        xIndex[yIndex[i]].second = i;

    return sqrt(ShortestDistance(points, xIndex, yIndex, 0, points.size() - 1));
}
...
double ShortestDistance_v2(const vector<pair<double, double>> &points)
{
    ...
    for (int i = 0; i < points.size(); i++)
    {
        ...
        {
            double aDistance = Distance2(points, i, j);
...
int main()
{
    ...
    cout << "穷法法计算结果: " << aShortestDistance_v2 << ", 共计算点距 " << _count << "次。" << endl;
